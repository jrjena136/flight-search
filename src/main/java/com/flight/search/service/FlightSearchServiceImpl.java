...
public class FlightSearchServiceImpl implements FlightSearchService {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.flight.search.service.IFlightSearchService#searchFlights(java.lang.
	...
	public String searchFlights(String origin, String destination) {
		...
		List<FlightsProviderDetails> filterFlights = flightProviders.stream()
				.filter(flightProvider -> flightProvider.getOrigin().contains(origin)
						&& flightProvider.getDestination().contains(destination))
		...
		if (null != filterFlights && !filterFlights.isEmpty()) {
			...
			filterFlights.forEach(flightBean -> flightResult.append(flightBean.getOrigin() + " --> "
					+ flightBean.getDestination() + " (" + DateUtils.parseDateToString(flightBean.getDepartureTime())
					+ " --> " + DateUtils.parseDateFormat(flightBean.getDestinationTime()) + " ) -  $"
					+ flightBean.getPrice() + "0").append("\n"));
		} else {
			flightResult.append(NO_FLIGHT + origin + " --> " + destination);
		}
		logger.info("Total Searched Flights : "  +flightResult);
